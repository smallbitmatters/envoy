...
class Message:
    ...
    def __init__(self):
        ...
        self.headers = [(f'header_{str(h)}', os.urandom(128)) for h in range(3)]
...
class IntegrationTest(unittest.TestCase):
    ...
    def kafka_envoy_address(cls):
        return f'127.0.0.1:{IntegrationTest.services.kafka_envoy_port}'

    @classmethod
    def kafka_cluster1_address(cls):
        return f'127.0.0.1:{IntegrationTest.services.kafka_real_port1}'

    @classmethod
    def kafka_cluster2_address(cls):
        return f'127.0.0.1:{IntegrationTest.services.kafka_real_port2}'

    @classmethod
    def envoy_stats_address(cls):
        return f'http://127.0.0.1:{IntegrationTest.services.envoy_monitoring_port}/stats'
...
class ServicesHolder:
    ...
    def start(self):
        ...
        kafka_store_dir1 = f'{self.kafka_tmp_dir}/kafka_data1'
        os.mkdir(kafka_store_dir1)
        kafka_store_dir2 = f'{self.kafka_tmp_dir}/kafka_data2'
        ...
        while True:
            ...
            with open(kafka_config_file1, 'w') as fd:
                fd.write(contents)
                print(f'Kafka config file rendered at: {kafka_config_file1}')
            ...
            with open(kafka_config_file2, 'w') as fd:
                fd.write(contents)
                print(f'Kafka config file rendered at: {kafka_config_file2}')
